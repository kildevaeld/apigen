...
pub struct PluginManager2<T: Plugin> {
    plugins: Vec<Entry2<T>>,
    loaded_libraries: Vec<LibraryEntry>,
}

impl<T: Plugin> PluginManager2<T> {
    pub fn new() -> PluginManager {
        PluginManager {
            plugins: Vec::new(),
            loaded_libraries: Vec::new(),
    ...
    pub unsafe fn load_plugin<P: AsRef<OsStr>>(&mut self, filename: P) -> Result<&Box<T>> {
        type PluginCreate = unsafe fn() -> *mut Plugin;
        ...
        let plugin = Box::from_raw(boxed_raw as *mut T);
        debug!("Loaded plugin: {}", plugin.name());
        plugin.on_plugin_load();

        self.plugins.push(Entry2 {
            id: id,
            path: String::from(filename.as_ref().to_str().unwrap()),
            plugin: plugin,
        });
    ...
    pub fn plugin_with_name(&self, name: &str) -> Vec<&Box<T>> {
        let mut out = vec![];
        for plugin in &self.plugins {
            if plugin.plugin.name() == name {
                out.push(&plugin.plugin);
    ...
    pub fn plugin(&self, id: Uuid) -> Option<&Box<T>> {
        for plugin in &self.plugins {
            if plugin.id == id {
                return Some(&plugin.plugin);
            }
...
impl<T: Plugin> Drop for PluginManager2<T> {
    fn drop(&mut self) {
        self.unload();
    }
}
